...
/**
 * The template for displaying archive Featured Posts section
 *
 * @package Roven-Blog
 */
$count = get_theme_mod( 'rovenblog_archive_featured_nr', 3 );
$posts_type = get_theme_mod( 'rovenblog_archive_featured_content', 'recent-posts' );
$args_featured = rovenblog_posts_args(
    $posts_type,
    $count,
    'rovenblog_archive_featured',
    false
);
if ( true === get_theme_mod( 'rovenblog_archive_featured_query_var', true ) ) {
    // Restrict the section loop to post from the queried catgory or tat.
    
    if ( is_category() ) {
        $args_featured['category__in'] = array( get_queried_object()->term_id );
    } elseif ( is_tag() ) {
        $args_featured['tag__in'] = array( get_queried_object()->term_id );
    }
...
if ( $query_posts->have_posts() ) {
    // Archive/Category Featured section Customizer arguments.
    $section_title = get_theme_mod( 'rovenblog_archive_featured_title', esc_html__( 'Featured posts', 'roven-blog' ) );
    $nr_cols = intval( get_theme_mod( 'rovenblog_archive_featured_cols', 3 ) );
    // Archive/Category Featured posts Customizer arguments.
    $style_type = get_theme_mod( 'rovenblog_archive_featured_style', 'style3' );
    $category_enable = get_theme_mod( 'rovenblog_archive_featured_category', true );
    $excerpt_enable = get_theme_mod( 'rovenblog_archive_featured_excerpt', false );
    $thumbnail_type = get_theme_mod( 'rovenblog_archive_featured_aspect', 'portrait' );
    ...
    if ( 'style1' === $style_type || 'style2' === $style_type ) {
        ...
        $icons_enable = false;
        $author_enable = get_theme_mod( 'rovenblog_archive_featured_author', true );
        $date_enable = get_theme_mod( 'rovenblog_archive_featured_date', true );
        $comments_enable = get_theme_mod( 'rovenblog_archive_featured_comments', true );
        $args = array(
            'category_enable' => $category_enable,
            'thumbnail_type'  => $thumbnail_type,
            'thumbnail_limit' => $thumbnail_size,
    ...
    ?>
	<div id="rb-archive-featured-posts" class="rb-section<?php 
	...
	</div><!-- end #rb-archive-featured-posts -->
	<?php 
}
